...
```shell
python examples/qwen_grpo_tutorial/rlhf_data.py \
...
cd /{path}/mindrlhf
```
使用以下命令将本地mindrlhf和mindformers代码库均加入PYTHONPATH路径中
```shell
expert PYTHONPATH="/{path}/mindrlhf/:$PYTHONPATH"
expert PYTHONPATH="/{path}/mindformers/:$PYTHONPATH"
```
