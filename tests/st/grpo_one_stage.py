...
def main(sft_path_infer, sft_path_train, use_parallel, args):
    ...
    if get_rank() in list(range(0, get_group_size(), get_group_size() \
    ...
    grpo_config.num_rollouts = 1
    grpo_config.num_generations = 2

    grpo_config.epochs = 1
    for n in range(grpo_config.epochs):
        # do generate
        steps = 1
        for i in range(steps):
            print(f"--------- epoch:{n} step:{i} ---------")
            trainer.make_experience(num_rollouts=grpo_config.num_rollouts,
