...
class TestQwen2DPO:
    """A test class for testing qwen2 dpo"""
    ...
    def test_qwen2_dpo_process(self):
        ...
        self.change_yaml(os.path.join(root_path, "model_configs", "qwen_config", "process_qwen2_7b.yaml"))
        ...
        scripts_cmd = f"{scripts_path} --src={dpo_data_path} " + \
                      f"--dst={sh_path}/qwen2.mindrecord " + \
        ...
        os.system(f"python {root_path}/dpo_preprocess.py \
                    ...
                    --dst {sh_path}/qwen2.mindrecord")
        reader = FileReader(file_name=f"{sh_path}/qwen2.mindrecord")
        ms_len = reader.len()
        reader.close()
        ...
        assert os.path.isfile(f"{sh_path}/qwen2.mindrecord")
    ...
    def test_qwen2_finetune(self):
        ...
        self.change_yaml(os.path.join(root_path, "model_configs", "qwen_config", "finetune_qwen2_7b_dpo.yaml"))
        scripts_cmd = f"{scripts_path} --config={root_path}/model_configs/qwen_config/finetune_qwen2_7b_dpo.yaml " + \
                      f"--run_mode=finetune " + \
                      f"--train_dataset={sh_path}/qwen2.mindrecord "
